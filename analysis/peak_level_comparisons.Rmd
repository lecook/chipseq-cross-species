...
conserved_peaks_enhancer = rbindlist(summarise_liftover(file_pattern= "*_mm10intersect_dunnart_enhancer_50bpsummits.bed",
                                     files = 1:6, column_name = "conserved peaks"), )

conserved_peaks_enhancer <- conserved_peaks_enhancer[
  ,`total dunnart peaks`:= rep(35676, 6)
  ][
    ,`total dunnart to mouse alignable peaks with annotation`:= rep(4313, 6)
    ]

conserved_peaks_promoter = rbindlist(summarise_liftover(file_pattern= "*_mm10intersect_dunnart_promoter_50bpsummits.bed",
                                     files = 1:6, column_name = "conserved peaks"), )

conserved_peaks_promoter <- conserved_peaks_promoter[
  ,`total dunnart peaks`:= rep(8510, 6)
  ][
    ,`total dunnart to mouse alignable peaks with annotation`:= rep(1442, 6)
    ]

conserved_peaks <- list(conserved_peaks_enhancer, conserved_peaks_promoter)

conserved_peaks <- lapply(conserved_peaks, function(x){
  x <- x[
    ,stage:=c("E10.5", "E11.5", "E12.5", "E13.5", "E14.5", "E15.5")
  ][
    ,`% conserved peaks of alignable peaks`:=(as.numeric(`conserved peaks`) / 
                                                as.numeric(`total dunnart to mouse alignable peaks with annotation`))*100
    ][
      ,`% conserved peaks of total dunnart peaks`:=(as.numeric(`conserved peaks`) / 
                                                as.numeric(`total dunnart peaks`))*100
      ][
        ,c(4,2,3,1,5,6)
        ]
})                              

kable(conserved_peaks[[1]], caption="Summary of mouse-dunnart conserved enhancer-associated peaks")
kable(conserved_peaks[[2]], caption="Summary of mouse-dunnart conserved high-confidence promoter-associated peaks")
```
