...
setwd("output/qc/ucsc_alignment/")
```

### Summarise statistics from MafFilter

```{r}
mm10 <- fread("mm10.statistics.csv") # read in table

smiCra1 <- fread("smiCra1.statistics.csv") # read in table
...
```{r}
mm10exon <- fread("mm10exonsMerged.bed") ## exon file
mm10exon$length <- (mm10exon$V3 - mm10exon$V2) ## exon length in ncbi refseq 
mm10overlaps <- fread("mm10exonOverlaps.bed") ## overlap file
...
```{r}
smiCra1exon <- fread("output/qc/ucsc_alignment/smiCra1exonsMerged.bed", sep="\t", header=FALSE) ## exon file
dim(smiCra1exon) ## check file imported correctly
smiCra1exon$length <- (smiCra1exon$V3 - smiCra1exon$V2)  ## exon length
smiCra1overlaps <- fread("output/qc/ucsc_alignment/smiCra1_exonOverlaps.bed", sep="\t", header=FALSE) ## exon overlaps with maf alignment file
...
```{r}
mm10 <- fread("output/qc/ucsc_alignment/mm10.divergence.statistics.csv", header=TRUE, sep="\t")
smiCra1 <- fread("output/qc/ucsc_alignment/smiCra1.divergence.statistics.csv", header=TRUE, sep="\t")

mean(mm10$Div.mm10.smiCra1)
mean(smiCra1$Div.smiCra1.mm10)

## Genome coverage
readDNAStringSet("data/genomic_data/mm10_mafAlignment_nogaps.fasta") #number bp in alignment 743350114
readDNAStringSet("data/genomic_data/smiCra1_mafAlignment_nogaps.fasta") #number bp in alignment 747935780
...
print(pasteo("dunnart genome coverage =", smiCra1coverage))
```

## LiftOver

```{bash eval=FALSE}
liftOver data/genomic_data/smiCra1exonsMerged.bed output/wga/liftover_chains/mm10_smiCra1_patched_sorted_repFil_chainCl_preNet_chainNet_stitched.chain smiCra1exonsMapped.txt output/qc/ucsc_alignment/smiCra1exonsUnmapped.txt
```
