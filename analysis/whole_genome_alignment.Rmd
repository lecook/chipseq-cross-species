...
setwd("output/qc/ucsc_alignment_qc/maf_filter/")
```
...
print("mm10 blocks per chromosome",mm10ChrCount)
print("dunnart blocks per scaffold", smiCra1ChrCount)
```
...
df <- data.table(
  ...
  sum = c(sum(size1to10bp$BlockLength),
  sum(size10to100bp$BlockLength),
  sum(size100to1kb$BlockLength),
  sum(size1kbto10kb$BlockLength),
  sum(size10kbto100kb$BlockLength),
  sum(size100kbto1Mb$BlockLength))
  count = c(count(size1to10bp), count(size10to100bp), count(size100to1kb), 
  count(size1kbto10kb), count(size10kbto100kb), count(size100kbto1Mb))
)
...
```{r}
system("bedtools sort -i output/qc/ucsc_alignment_qc/mm10exons.bed > output/qc/ucsc_alignment_qc/mm10exonsSorted.bed", intern=TRUE) ## sort by coordinates

system("bedtools merge -i output/qc/ucsc_alignment_qc/mm10exonsSorted.bed > output/qc/ucsc_alignment_qc/mm10exonsMerged.bed", intern=TRUE) ## bedtools merge where transcripts are the same coordinates or within each other

system("bedtools merge -i output/qc/ucsc_alignment_qc/mm10_mafAlignment.bed > output/qc/ucsc_alignment_qc/mm10_mafAlignmentMerged.bed", intern=TRUE) ## also merge the maf alignments from the liftOver

system("bedtools intersect -a output/qc/ucsc_alignment_qc/mm10exonsMerged.bed -b output/qc/ucsc_alignment_qc/mm10_mafAlignmentBlocks.bed -wo > output/qc/ucsc_alignment_qc/mm10exonOverlaps.bed", intern=TRUE) ## intersect exon coordinates with maf alignment file
```
...
```{r}
system("bedtools sort -i output/qc/ucsc_alignment_qc/dunnart_exons.bed > output/qc/ucsc_alignment_qc/dunnart_exonsSorted.bed", intern = TRUE) ## sort by coordinates

system("bedtools merge -i output/qc/ucsc_alignment_qc/dunnart_exonsSorted.bed > output/qc/ucsc_alignment_qc/smiCra1exonsMerged.bed", intern = TRUE) ## bedtools merge where transcripts are the same coordinates or within each other

system("bedtools merge -i output/qc/ucsc_alignment_qc/smiCra1_mafAlignmentBlocksSorted.bed > output/qc/ucsc_alignment_qc/smiCra1_mafAlignmentMerged.bed", intern = TRUE) ## also merge the maf alignments from the liftOver
system("bedtools intersect -a output/qc/ucsc_alignment_qc/smiCra1exonsMerged.bed -b output/qc/ucsc_alignment_qc/smiCra1_mafAlignmentMerged.bed -wo > output/qc/ucsc_alignment_qc/smiCra1_exonOverlaps.bed", intern=TRUE) ## intersect exon coordinates with maf alignment file
...
smiCra1coverage <- 747935780/smiCra1genomeSize

print("mm10 genome coverage = ", mm10coverage)
print("dunnart genome coverage =", smiCra1coverage)
```
